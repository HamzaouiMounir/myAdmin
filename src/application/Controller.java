...
public class Controller implements Initializable {
	...
	@FXML
	public MenuItem close,licence,about;
	@FXML
	...
	public void initialize(URL location, ResourceBundle resources) {
		try {
			serverSocket = new ServerSocket(2999, 200, InetAddress.getByName(getLocalAddress()));
	   ...
	   /* final WebEngine webEngine = cmdResult.getEngine();
			...
			public void changed(ObservableValue<? extends Socket> observable, Socket oldValue, Socket newValue) {
				...
				try {
					...
					BufferedReader in = new BufferedReader(new InputStreamReader(s.getInputStream()));
			 		...
			 		showErrorDialog(conHosts.getSelectionModel().getSelectedItem());
		...
		licence.setOnAction(new EventHandler<ActionEvent>() {

			@Override
			public void handle(ActionEvent event) {
				Alert alert = new Alert(AlertType.CONFIRMATION);
				alert.setTitle("MyAdmin 1.0");
				alert.setHeaderText("MyAdmin vous dispose une licence gratuite");
				alert.setContentText("myAdmin est en version Beta");
		...
		about.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				Alert alert = new Alert(AlertType.CONFIRMATION);
				alert.setTitle("MyAdmin 1.0");
				alert.setHeaderText("MyAdmin est développé par Mounir Hamzaoui étudiant à EPI Sousse");
				alert.setContentText("myAdmin est en version Beta et sera bientôt disponible sur guithub pour plusieurs contribution");
				alert.show();
				alert.showAndWait().ifPresent(rs -> {
				    if (rs == ButtonType.OK) {
				    		alert.close();
	...
	public void showErrorDialog(Socket s){
		try {
			if(!s.isClosed())
			s.close();
		} catch (IOException e) {
		 ...
		 mngmnt.setDisable(true);
		Alert alert = new Alert(AlertType.ERROR);
		alert.setTitle("Alerte");
		alert.setHeaderText("Erreur de connexion");
		alert.setContentText("Cette machine est hors connexion , vous pouvez plus l'administrer");
	...
	public Thread getRealTimeConnectedHosts(ListView<Socket> conHosts,Label online){
		Task<Void> task = new Task<Void>() {
			...
			protected Void call() throws Exception {
				...
				while(true){
					/***/
						...
						System.out.println("Hey for client on port " + serverSocket.getLocalPort() + "...");
	...
	public Thread checkConnectivity(ListView<Socket> conHosts, Label onH, Label offH, Label date, Label time){
		Task<Void> task = new Task<Void>() {
					...
					conHosts.setCellFactory(new Callback<ListView<Socket>, ListCell<Socket>>() {
						...
						public ListCell<Socket> call(ListView<Socket> arg0) {
							...
							for (int i = 0; i < arg0.getItems().size(); i++) {
								Socket s = arg0.getItems().get(i);
								try {
									if(!InetAddress.getByName(s.getInetAddress().getHostAddress()).isReachable(5000)){
										s.close();
									}
								} catch (UnknownHostException e) {
									// TODO Auto-generated catch block
